...
int main()
{
    ...
    Asset Sword ("Sword", "A sword to help defend yourself", 150, true);
    Asset purplehaze("Purple haze", "A spell that renders opponents helpless.", 250, true);
    Asset shield("Shield", "Throw at an opponent .", 100, true);
    ...
    gameMap[randNode].AddAsset(&Sword);

    randNode = rand() % numOfNodes;
    gameMap[randNode].AddAsset(&purplehaze);

    randNode = rand() % numOfNodes;
    gameMap[randNode].AddAsset(&shield);
    ...
    Monster witch("witch", 7000, 100);
    Monster demon("demon", 5000, 100);
    Monster gargoyle ("gargoyle", 4000, 100);
    ...
    randNode = rand() % numOfNodes;
    gameMap[randNode].AddMonster(&witch);

    randNode = rand() % numOfNodes;
    gameMap[randNode].AddMonster(&demon);

    randNode = rand() % numOfNodes;
    gameMap[randNode].AddMonster(&gargoyle);
   ...
   // if player wants to take an asset (f flashlight)
        if (input.length() > 1 && input[0] == 'f')
        {
            string lastWord = getLastWord(input);
            for(Asset *assets : gameMap[nodePointer].GetAssets())
            {
                if(assets->GetName() == lastWord)
                {
                    player.AddAsset(assets);
                    // adding an asset to a player
                }
            }
        }

        // if player wants to attack a monster (s Sword)
        ...
        // if player wants to drop an asset (p PurpleHaze)
        ...
        // if player wants to inspect an asset (d Drinking Water)
